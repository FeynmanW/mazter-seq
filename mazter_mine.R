...
# Functions ############################################################
source("https://raw.githubusercontent.com/FeynmanW/mazter-seq/master/helperFunctions.R")

# Packages ############################################################
# CRAN packages
CRAN_packages <- c("magrittr", "parallel", "stringr", "RColorBrewer", "ggplot2", "grid", "gtools")
...
tmpFName <- tempfile()
system(paste0("bedtools getfasta -s -split -fi ", faGenome, " -bed ", geneAnnotation, " > ", tmpFName, ".fseq.fa"))
...
txOmeSEQS <- sapply(txOmeSEQS, str_to_upper)
